...
public class CypherByMusic  {
	...
	private View.OnClickListener bLis;

	public CypherByMusic(final Context cxt) {
		this.cxt = cxt;


		config = GNConfig.init("7486464-12AF0CC1BCE8C9726F6ADC0F77D3AF6D",cxt.getApplicationContext());
		...
		bLis = new View.OnClickListener() {
			public void onClick(View v) {
				RecognizeFromMic task = new RecognizeFromMic();
				task.doFingerprint();

				((Activity) cxt).findViewById(R.id.progressBarEncodeButton).setVisibility(View.VISIBLE);
	...
	class RecognizeFromMic implements GNSearchResultReady {
		...
		public void GNResultReady(GNSearchResult result) {
			...
			key_textview = (TextView) ((Activity) cxt).findViewById(R.id.keySongTextView);
			msg_ori = (EditText) ((Activity) cxt).findViewById(R.id.messageTextView);
			msg_encoded = (TextView) ((Activity) cxt).findViewById(R.id.criptedMessageTextView);

			if (result.isFingerprintSearchNoMatchStatus()) {
				//song_info.setText("no match");
			} else {
				GNSearchResponse response = result.getBestResponse();
				Log.i("HHHearts", " Response " + response.getTrackTitle() + " " + response.getArtist() );
				...
				JsonObjectRequest jsObjRequest = new JsonObjectRequest(Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						...
						((Activity) cxt).findViewById(R.id.progressBarEncodeButton).setVisibility(View.GONE);
